...
export default class CountDown extends Component {
    ...
    _handleAppStateChange = currentAppState => {
        ...
        if (currentAppState === 'active' && wentBackgroundAt && this.props.running) {
            const diff = (Date.now() - wentBackgroundAt) / 1000;
            this.setState({ lastUntil: until, until: Math.max(0, until - diff) });
        }
    ...
    renderDoubleDigits = (label, digits) => {
        return (<View style={styles.doubleDigitCont}>
    ...
    render() { return (<View style={this.props.style}>{this.renderCountDown()}</View>); }
}
